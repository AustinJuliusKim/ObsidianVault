...
## Status
MVP (single commit); functional for single/multi-prompt sessions. Direction locked: [[Claude REPL Business Plan]] (v1.1 — guided free tier + metered wallet) and [[Claude REPL Lesson Plan]] (v1.0 — 8-lesson guided spine). Phase A/B technical architecture done: [[Claude REPL Architecture]] (v1.0). **Phase A MVP rebuilt greenfield (2026-07-02)** at `projects/{packages/guided-repl-protocol, services/guided-repl-seeder, apps/guided-repl}` — the prior claude-repl codebase was deleted; the notes [[Claude REPL Frontend]]/[[Claude REPL Backend]]/[[Claude REPL Protocol]] describe that deleted MVP and are historical. Greenfield MVP with real recorded fixtures (local `claude -p` runner; E2B seam stubbed). **All 8 lessons live (2026-07-03)**: spine v1.1 recorded (~20 live runs, snapshot chain l1→l8), quiz assertions + L2 step-through annotations, CLI-style UI (⏺/⎿ transcript, explorer tree) — arch doc v1.1 amendments. 163 unit tests + 17 Playwright e2e, CI + DAG↔fixtures/redaction gates. Deviation from arch doc: frames are nested `{type, payload}` (see FIXTURE_FORMAT.md). File preview shipped 2026-07-03 ([[Claude REPL File Preview]] P-A/P-B: sandboxed HTML+Markdown preview tabs; JS rendering = P-C later) + lesson-kind rail chips (quiz/check) + file-tree hover fix; 74 unit / 19 e2e. **Accounts & Progress implemented (2026-07-12)** on branch `feature/repl-accounts-progress` (PR #27, merged 2026-07-13): `Capture` step + `{{userName}}` render-time interpolation across protocol/engine/app, L1 name (post-intro) + email (post-grade) capture moments, tokenized fixtures (default persona "Demo User"), and new `services/guided-repl-api` — Fastify + pg behind a Supabase auth adapter, SQL migrations (users/leads/progress/events/sessions + append-only `wallet_ledger` w/ GDPR carve-out), deployed as Lambda via `@fastify/aws-lambda` behind a conditional CloudFront `/api/*` behavior (Dockerfile kept for the spec's container topology); CI `api` job added. e2e proves guided mode never blocks on the API (30/30 with no backend). Ops completed 2026-07-12 evening: Supabase project `gyxtrrwvfseyshjxeeyr` live + migrated, `GuidedReplApi` stack deployed (manual, admin), `ApiOriginDomain` wired, `learn.austinjuliuskim.com/api/health` 200. Follow-ups landed 2026-07-13: fixture v1→v2 cache-bust bump merged (PR #30); CI `api-deploy` job + deploy-role policy doc + magic-link email template merged (PR #31) — deploy-role IAM policy applied live, magic-link template + SES sandbox SMTP (domain DKIM verified) pasted into Supabase Auth. **Prod incident found+fixed same day:** DB-touching endpoints had been 500ing since the 07-12 deploy (`ENOTFOUND tenant/user <ref>` — Supavisor rejecting the deployed `DatabaseUrl`'s pooler creds even though `SupabaseUrl`'s ref matched; `/api/health` stayed green throughout since it never touches Postgres). Fixed via a parameter-only CloudFormation update (no rebuild) redeploying `DatabaseUrl` to the correct transaction-pooler (6543) connection string; `/api/progress` confirmed 200 live. **Lesson Foundry implemented 2026-07-13** per [[Claude REPL Lesson Foundry Spec]] (PR pending): six-stage authoring pipeline on GitHub Actions cron, draft-PRs-only with structural publish guard, `foundry bench` Model Lab, keyless CI — ops before first run: ANTHROPIC_API_KEY/E2B_API_KEY secrets, E2B template with claude CLI, branch protection + foundry:draft label, anthropic-news URL check, supervised first run. Next: live BYOK transport; E2B runner; advanced-track content; `--resume` continuity beat; preview P-C.
