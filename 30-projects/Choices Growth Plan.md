...
1. **Tier-1 hardening (§10, §10b)** — ✅ deployed to prod 2026-07-04 (origin-lock + WAF Block flips pending soak); canary/EMF observability follows; everything else in §10 waits for a growth trigger. **EMF observability partially built 2026-07-09** (PR #26): custom metrics via EMF from the Lambda (per-action latency p50/p95/p99 + business-heartbeat counters — games created/completed/seat-claimed/fill) and the CloudWatch **ops dashboard** (golden signals), the dashboard in a separate admin-deploy stack `ops/ops-dashboard.yaml` (CI role lacks `cloudwatch:PutDashboard`). Shipped alongside an owner-only anonymous-aggregate activity dashboard at `#/admin`. Still pending from §10: Synthetics canary, business/funnel dashboard, frontend Sentry/RUM — see [[Choices Webapp]]
