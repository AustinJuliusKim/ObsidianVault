...
## Changelog
- **First run + tuning** (2026-07-14) — First supervised `radar` run (workflow_dispatch) succeeded, $0.84/$10 cap: E2B template, the `htmlList` `anthropic-news` source (2 bench triggers fired), and PAT/CI wiring all validated live. Opened 0 lesson bundles — all 3 authored candidates correctly failed validate+seed (the seed gate working), which surfaced two systemic author bugs, now fixed on branch `worktree-foundry-tuning`: (1) **lesson `order` collision** — the author copies the l1 exemplar's `order: 1`; added a deterministic next-free-order rewrite of the draft YAML before validation (`src/lessons/nextOrder.js`, applied in `run.js`/`bench.js`); (2) **`error_max_turns`** — `maxTurns` was hardcoded to 1; added an `authorMaxTurns` settings knob (default 3) threaded through `agentClient`, and the author retry now covers SDK errors too (not just bad output). 92 foundry tests keyless (+9).
- **Merged + ops** (2026-07-13) — PR #32 merged to `main`. `anthropic-news` deviation resolved: Anthropic has no official RSS feed (`news/rss.xml` 404s) → switched to `htmlList` scrape of `anthropic.com/news` (bench trigger preserved — `benchTrigger` is decoupled from fetch method). E2B template Dockerfile scaffolded (`services/guided-repl-seeder/e2b.Dockerfile`, publish alias `guided-repl-seeder`) → PR #36. Repo ops done: `ANTHROPIC_API_KEY` + `E2B_API_KEY` secrets set, `foundry:draft`/`foundry:radar` labels created, Actions→create-PRs enabled, `main` branch protection (PR-required, admin-bypass). Remaining manual ops: build/verify the E2B template + one supervised first `workflow_dispatch` run.
