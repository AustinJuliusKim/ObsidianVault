...
| ✅ 12 | Feature flags: `#/admin` + client hooks — **done 2026-07-23, PR projects#56 (draft, stacked on #55)**: admin group via CFN (both stacks), group-claim-gated #/admin with 404 fallback, code-split AdminScreen (bundle-inspected), FlagsPanel with 409 surfacing, FlagsProvider/useFlag (no first-paint block; flagsCore unit-tested, frontend tests now in CI). Live acceptance walk rides #56 Ops tasks | Sonnet | 1–2d | Prompt 11 |
...
- ✅ **H-2 answered 2026-07-23** (ops run, admin creds): zero pre-existing canaries, RUM monitors; only the known `ChoicesWebApp-activity` dashboard. No duplicates existed.
...
- ◐ **H-6 run 2026-07-23** (admin creds): dashboard redeployed (funnel widgets live), `ChoicesRum` deployed + `VITE_RUM_*` repo vars set (RUM activates on the next frontend deploy — confirm with a forced browser error), `CanarySecret` set on the app stack, `ChoicesCanary` deployed **daily 8:07am PT** (Austin's cost call, projects#57; ~$0.04/mo vs ~$10/mo at 5-min). Remaining: confirm the SNS email (check spam — expires 3 days), merge projects#57 before the first run (fixes a create-path suppression leak found live; 4 leaked test objects hand-deleted from both lake zones), first-canary-green check tomorrow. Original task list:  — run PR projects#54's Ops tasks in order: H-2 duplicate check → CanarySecret on the app stack → `ChoicesCanary` stack (+SNS confirm, canary green) → `ChoicesOpsDashboard` redeploy (funnel widgets render) → `ChoicesRum` stack + `VITE_RUM_*` repo variables + frontend redeploy (forced error appears in RUM). Acceptance for row 2 completes here.
